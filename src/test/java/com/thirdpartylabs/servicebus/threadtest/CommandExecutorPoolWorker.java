...
/**
 * Worker that runs a ChangeWombatVisibilityServiceBusCommand executor in a thread
 * Keeps track of visibility flags received via the commands so we can compare to those sent by the generator
 */
public class CommandExecutorPoolWorker extends AbstractWorker
{
    private final ServiceBus serviceBus = ServiceBus.getInstance();
    private final List<Boolean> visibilityFlags = new ArrayList<>();
    private final CommandSubscription<ChangeWombatVisibilityServiceBusCommand> subscription;

    /**
     * @param subscription A subscription for our command
     */
    public CommandExecutorPoolWorker(CommandSubscription<ChangeWombatVisibilityServiceBusCommand> subscription)
    {
