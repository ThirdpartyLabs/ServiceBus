...
/**
 * Worker that runs an AdditionServiceBusRequest responder for a RequestSubscriptionRendezvousChannel
 */
public class RequestResponderChannelWorker extends AbstractWorker
{
    private final ServiceBus serviceBus = ServiceBus.getInstance();
    private final List<int[]> calculations = new ArrayList<>();
    private final BlockingQueue<RequestSubscriptionRendezvousChannel<AdditionServiceBusRequest>> channelQueue;

    /**
     * @param subscription The subscription to respond to
     */
    public RequestResponderChannelWorker(RequestSubscription<AdditionServiceBusRequest> subscription)
    {
        this.channelQueue = subscription.getChannelQueue();
    }
